        ...
        if data != []:
            ...
            for u in range(1, len(data)):
                if(Scheduled[bestJob[1]] > Scheduled[data[u][1]]):
                    bestJob = data[u]
                elif Scheduled[bestJob[1]] == Scheduled[data[u][1]] and compareTardy(bestJob[0], bestJob[1], data[u][0], data[u][1], P, D, max(startTime, bestJob[2]), max(startTime, data[u][2]), J, Scheduled):
                    bestJob = data[u]
                elif Scheduled[bestJob[1]] == Scheduled[data[u][1]] and equalTardy(bestJob[0], bestJob[1], data[u][0], data[u][1], P, D, max(startTime, bestJob[2]), max(startTime, data[u][2]), J, Scheduled) and compareMakeSpan(bestJob[0], bestJob[1], data[u][0], data[u][1], P, len(MT), J):
                    bestJob = data[u]
